# BloodVital Synthetic Data Generation - Complete Usage Guide
...
```bash
python generate_synthetic_data.py --num-samples 100 --output "C:/Scratch/bloodvital/data/ml/test_100.json"
```

### Step 4: Generate Full Dataset (5,000+ samples)

```bash
python generate_synthetic_data.py --num-samples 5000 --output "C:/Scratch/bloodvital/data/ml/synthetic_lab_reports.json"
```
...
python example_training_pipeline.py \
  --input "C:/Scratch/bloodvital/data/ml/synthetic_lab_reports.json" \
  --output-dir "C:/Scratch/bloodvital/data/ml/" \
  --train-ratio 0.8 \
  --val-ratio 0.1 \
  --test-ratio 0.1
```
...
After training, integrate model into BloodVital:
...
const extractor = await pipeline('token-classification', 'bloodvital-tinybert-ner');
...
- **BloodVital Documentation**: C:\Scratch\bloodvital\README.md
...
4. Contact BloodVital development team
