# High-Accuracy Browser-Based Machine Learning for Medical Biomarker Extraction from Laboratory Reports

**Technical Report v1.0**

**Living Document Notice:** This technical report documents ongoing research and development. Content, methodologies, and results will evolve as R&D progresses. The current version represents a snapshot of the system at the time of publication. Check the repository for the latest updates.

**Important Disclaimer:** This is a technical report presenting proof-of-concept results based on preliminary validation with a limited real-world test set (n=22 international PDFs, 687 biomarkers; n=2 Australian PDFs, 76 biomarkers). "High-accuracy" refers to extraction accuracy against manually-verified ground truth labels, not clinical validation for medical decision-making. This system is designed for informational purposes and does not constitute medical advice, diagnosis, or treatment recommendations. Always consult qualified healthcare providers for interpretation of laboratory results.

**Authors:** Macdara Ó Murchú
**Affiliation:** Independent Researcher, Australia
**Date:** December 23, 2025
**Keywords:** Medical Informatics, Named Entity Recognition, Browser-Based Machine Learning, Clinical Document Processing, Privacy-Preserving AI
...
**Ethics Statement:** This research used synthetic training data and a small number of the author's own de-identified pathology reports for validation testing. No human subjects research was conducted beyond self-testing. No ethics approval was required.

**Conflict of Interest:** The author is the sole developer of BloodClarity, the application implementing this system.

**Funding:** This research received no external funding and was conducted independently.

**Data Availability:** Training data generation scripts, model architecture, and validation code are available at: https://github.com/m4cd4r4/bloodclarity-ml

**Code Availability:** Complete ML pipeline implementation, training scripts, and model code available in the same repository. The BloodClarity application repository (https://github.com/m4cd4r4/bloodclarity) contains proprietary implementation code and is not publicly available.
...
**Objective:** To develop a high-accuracy (≥98% extraction accuracy) biomarker extraction system deployable entirely in-browser with complete offline capability, addressing the trade-off between model performance and deployment constraints.

**Methods:** We implemented a five-component system combining: (1) synthetic training data generation (10,000 samples across 16 laboratory formats and 102 biomarkers), (2) multi-task learning architecture jointly optimising named entity recognition, format classification, and unit prediction, (3) multi-pass OCR preprocessing with error correction, (4) comprehensive biological plausibility validation, and (5) context-aware unit conversion. The system targets TinyBERT (14.5M parameters) optimised through knowledge distillation, INT8 quantization, and 50% pruning to achieve 12MB deployment size.

**Results:** System-level accuracy reached 98.8% on validation testing (n=22 international PDFs, 687 biomarkers) (raw ML: 97.6%, +biological validation: 0.8%, +context-aware units: 0.4%) across 102 biomarkers and 16 international laboratory formats. The optimised model achieves 45-80ms inference latency with 100% offline capability, representing a 10× speed improvement over cloud-based alternatives while eliminating recurring costs estimated at $162,000 annually for typical deployment scenarios.

**Conclusions:** High-precision medical NER is achievable in browser environments through domain specialisation, aggressive optimisation, and multi-modal validation. This approach enables privacy-preserving, cost-effective laboratory report processing at scales previously requiring enterprise infrastructure.

**Significance:** Our results suggest that in specialised medical domains with constrained vocabularies, trade-offs between model accuracy and deployment size may be less severe than previously assumed, providing one approach to privacy-compliant, offline-capable laboratory report processing.
...
This work addresses the research question: **Can high-accuracy (98% on validated samples) for medical biomarker extraction be achieved in a browser-deployable model (<15MB) with complete offline capability?**

We hypothesise that domain specialisation, synthetic data generation, multi-task learning, and aggressive optimisation can overcome the apparent trade-off between model accuracy and deployment constraints. Specifically, we propose that the constrained vocabulary (102 biomarkers vs. billions of general tokens), structured formats (16 laboratory templates), and predictable context (biological constraints) of laboratory reports enable compression techniques that would degrade performance on general-domain tasks.
...
2. **Dataset**: A novel synthetic training corpus of 10,000 laboratory reports spanning 16 international formats, 102 biomarkers, 6 languages, and 15% adversarial examples simulating OCR errors.
...
4. **Validation**: Comprehensive biological plausibility limits for 102 biomarkers preventing common extraction errors (e.g., laboratory accreditation numbers misinterpreted as sodium values).

5. **Deployment**: Evidence that high-precision medical NER may be achievable with 45-80ms inference latency in modern browsers via WebGPU/WASM, eliminating cloud dependencies.
...
**Quantization** reduces numerical precision from 32-bit floats (FP32) to 8-bit integers (INT8), yielding 4× size reduction with typical accuracy loss <1% [39,40]. Post-training quantization requires no retraining, while quantization-aware training minimises degradation [41].
...
No existing system combines high-accuracy (≥98%), browser-based deployment (<15MB), and complete offline capability for laboratory report processing. This gap may stem from the technical challenges of balancing accuracy and deployment size in general-domain models, which we attempt to address through domain-specific optimisation in the constrained space of laboratory reports.

*Note: Format and biomarker coverage is continuously expanding. Current implementation supports 16 formats and 102 biomarkers, with additional formats planned for future releases.*
...
Let $R = \{r_1, r_2, ..., r_n\}$ represent a set of laboratory reports, where each report $r_i$ contains textual representations of biomarker measurements in one of $F = 16$ laboratory formats.
...
Where:
- $name_j \in V_B$ (biomarker vocabulary, $|V_B| = 102$)
...
Where $f_{\theta}$ is a parameterised extraction function and $\hat{B}_i$ approximates ground truth $B_i$.
...
**C1: Format Diversity**: The 16 laboratory formats exhibit substantial variation:
...
Acquiring labelled laboratory reports presents dual challenges: patient privacy (HIPAA/GDPR) and format diversity (16 formats × 102 biomarkers = 1,632 combinations). Synthetic generation enables comprehensive coverage without privacy violations.
...
- **Test**: 2,000 samples
- **Formats**: 16 (balanced distribution)
- **Biomarkers**: 102 (long-tail distribution matching clinical frequency)
- **Languages**: 6
...
TinyBERT Encoder (14.5M parameters)
  ...
  ├─→ Format Head: Sequence classification (16 formats)
...
**Step 3: Contrast Enhancement** (Histogram equalisation)
...
**Pass 2**: Skip binarization (may help with coloured text)
...
Full database: 102 biomarkers with category-specific limits (Haematology, Metabolic, Lipids, Kidney, Liver, Thyroid, Vitamins, Electrolytes, Cardiac, Inflammation, Hormones, Iron Studies, Proteins, Autoimmune, Drug Monitoring, Tumor Markers, Blood Gases, Bone Health).
...
Normalisation via regex-based matching and lookup table.
...
**Quantization-aware training**: Fine-tune model with simulated quantization to minimise accuracy loss [41].
...
Structured pruning removes entire attention heads and feed-forward neurons, enabling actual speedup (vs. unstructured sparsity requiring specialised kernels).
...
| **Overall** | **16 formats** | **22** | **687** | **98.5%** | **98.6%** | **99.9%** | **99.2%** |

**Key Findings**:
- All formats achieved >97% accuracy (exceeding production-ready threshold)
...
|--------|------|----------|---------|---------|-----------------|
| **Ours (Optimised)** | **12MB** | **98.5%** | **48ms** | ✅ Offline | **$0** |
...
This proof-of-concept achieves 98.5% accuracy on a limited test set (n=22 PDFs, 687 biomarkers), suggesting that high accuracy may be achievable in browser environments for specialised medical NLP tasks. Domain specificity appears to be a key factor: laboratory reports exhibit constrained vocabulary (102 biomarkers), structured formats (16 templates), and predictable patterns that permit aggressive model compression without accuracy loss.
...
Complete offline operation may help address important privacy requirements:
...
Hypothetical cost savings compared to cloud alternatives (assuming 1,000 daily users, 3 PDFs/user):
...
**BloodClarity (Browser-based)**: $0 marginal cost

At enterprise scale (10,000 daily users), annual savings reach **$162,000 (Cloud OCR)** or **$540,000 (Commercial API)**.

### 7.4 Factors Contributing to the Results

**Factor 1: Narrow Domain**
102 biomarkers vs. billions of general vocabulary tokens. Specialised models outperform general models on narrow tasks [57].
...
Related tasks (NER, format, unit) share representations, improving generalisation [52].
...
**L2: Format Coverage**
16 formats represent major international labs but miss regional/institutional variations. Continuous expansion required.
...
This approach applies to other specialised medical NLP tasks with:
...
The preliminary results presented in this technical report suggest several directions for continued development:

### 9.1 Expanded Validation

**Larger Test Set**: Current validation (n=22 international PDFs, 687 biomarkers) represents proof-of-concept. Statistical rigour would require at minimum 500+ real-world documents with confidence intervals, cross-validation, and significance testing.

**Geographic Coverage**: Expand testing to underrepresented regions (Africa, Middle East, Eastern Europe) to validate format detection and extraction accuracy across diverse laboratory systems.

**Physician Validation**: Survey clinicians on extraction quality and clinical utility to assess real-world applicability beyond automated metrics.

### 9.2 Model Improvements

**OCR Enhancement**: Replace Tesseract.js with modern deep learning alternatives (e.g., TrOCR) if model size constraints can be addressed through further compression techniques.

**Active Learning**: Implement confidence-based flagging to identify uncertain extractions for manual review, using feedback to iteratively improve pattern matching and biological validation rules.

**Continuous Refinement**: Monitor real-world usage patterns to identify extraction failures and edge cases requiring format-specific improvements.

### 9.3 Format Extensions

**Additional Providers**: Expand beyond current 16 formats to cover additional international laboratories and specialised testing domains (pharmacogenomics, genetic panels, advanced metabolic testing).

**Multi-Modal Processing**: Incorporate graphical data (trend charts, reference range visualisations) often embedded in modern digital pathology reports.

### 9.4 Community Contributions

As an open-source project (github.com/m4cd4r4/bloodclarity-ml), community contributions may help address:
- Laboratory format definitions for additional providers
- Improved biological validation rules for emerging biomarkers
- Enhanced OCR preprocessing pipelines for challenging PDF structures
- Internationalisation and localisation for non-English reports

### 9.5 Research Directions

**Transfer Learning**: Investigate whether pre-training on synthetic data and fine-tuning on small real-world samples can improve few-shot learning performance.

**Explainability**: Develop attention visualisation to show which text regions influenced extractions, building trust for clinical adoption.

**Fairness Analysis**: Evaluate accuracy across demographic groups and geographic regions to detect and mitigate potential bias.

**Note**: All enhancements must maintain the privacy-first, browser-based architecture that defines this approach.

---

## 10. Conclusion

This proof-of-concept achieves 98.5% accuracy on preliminary validation (n=22 PDFs, 687 biomarkers), suggesting that high-precision medical NER may be feasible in browser environments through domain-specific optimisation. Our five-component system—synthetic data generation, multi-task learning, OCR preprocessing, biological validation, and context-aware unit conversion—provides evidence that trade-offs between model accuracy and deployment constraints may be less severe in specialised domains than previously assumed.

Key contributions include:

1. **12MB optimised model** achieving 97.6% raw ML accuracy through distillation, quantization, and pruning
2. **System-level 98.8% accuracy** via multi-modal validation (biological plausibility + context-aware units)
3. **10,000-sample synthetic corpus** spanning 16 formats, 102 biomarkers, and 6 languages with adversarial examples
...
This approach generalises to other specialised medical NLP tasks with constrained vocabularies and structured formats. Future work includes expanded validation across 10,000+ real-world reports, technical enhancements (deep learning OCR, mobile optimisation), and deployment modes (browser extension, EHR integration).

**Significance**: By demonstrating that high accuracy can be achieved in privacy-preserving browser deployment, this work provides evidence that barriers to AI-assisted laboratory report processing may be surmountable. If validated on larger datasets, such approaches could give patients control over health data while providing automated extraction tools previously limited to enterprise cloud systems.

Our results suggest that in specialised medical domains with constrained vocabularies and structured formats, the trade-off between model accuracy and deployment constraints may be less severe than in general-domain applications. This work may inform future approaches to privacy-preserving medical NLP across radiology reports, pathology results, and other structured clinical documents.
...
[47] Nikolenko SI. Synthetic Data for Deep Learning. *Springer Optimization and Its Applications*. 2021.
...
```
Input: Biomarker database B (102 biomarkers), Format templates F (16 formats), Sample count N
...
Full database: 102 biomarkers available in the validation module.
...
Format Head:
  ...
  ReLU
  Dropout(0.1)
  Linear(256 → 16)        // 16 format classes
  Parameters: 84,000
